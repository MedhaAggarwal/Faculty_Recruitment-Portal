...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Retrieve form data for publications_summary
    ...
    // Prepare and bind SQL statement for publications_summary
    $stmt = $conn->prepare("INSERT INTO publications_summary (summary_journal_inter, summary_journal, summary_conf_inter, summary_conf_national, patent_publish, summary_book, summary_book_chapter) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $summary_journal_inter, $summary_journal, $summary_conf_inter, $summary_conf_national, $patent_publish, $summary_book, $summary_book_chapter);

    // Execute statement for publications_summary
    if (!$stmt->execute()) {
    ...
    // Retrieve form data for publications
    ...
    // Prepare and bind SQL statement for publications
    $stmt = $conn->prepare("INSERT INTO publications (author, title, journal, year, impact, doi, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $author, $title, $journal, $year, $impact, $doi, $status);

    // Insert each publication into the database
    for ($i = 0; $i < count($authors); $i++) {
        ...
        // Execute statement for publications
        if (!$stmt->execute()) {
            echo "Error: " . $stmt->error;
    ...
    // Close statement for publications
    $stmt->close();

    // Handle Patent(s) data
    if(isset($_POST['pauthor']) && !empty($_POST['pauthor'])) {
        $pauthors = $_POST['pauthor'];
        $ptitles = $_POST['ptitle'];
        $pcountries = $_POST['p_country'];
        $pnumbers = $_POST['p_number'];
        $pyears_filed = $_POST['pyear_filed'];
        $pyears_published = $_POST['pyear_published'];
        $pyears_issued = $_POST['pyear_issued'];

        // Prepare and bind SQL statement for Patent(s)
        $stmt = $conn->prepare("INSERT INTO patents (author, title, country, number, year_filed, year_published, year_issued) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $pauthor, $ptitle, $pcountry, $pnumber, $pyear_filed, $pyear_published, $pyear_issued);

        // Insert each patent into the database
        for ($i = 0; $i < count($pauthors); $i++) {
            $pauthor = $pauthors[$i];
            $ptitle = $ptitles[$i];
            $pcountry = $pcountries[$i];
            $pnumber = $pnumbers[$i];
            $pyear_filed = $pyears_filed[$i];
            $pyear_published = $pyears_published[$i];
            $pyear_issued = $pyears_issued[$i];

            // Execute statement for Patent(s)
            if (!$stmt->execute()) {
                echo "Error: " . $stmt->error;
                break; // Stop inserting if an error occurs
            }
        ...
        // Close statement for Patent(s)
        $stmt->close();
    }

    // Handle Book(s) data
    if(isset($_POST['bauthor']) && !empty($_POST['bauthor'])) {
        $bauthors = $_POST['bauthor'];
        $btitles = $_POST['btitle'];
        $byears = $_POST['byear'];
        $bisbns = $_POST['bisbn'];

        // Prepare and bind SQL statement for Book(s)
        $stmt = $conn->prepare("INSERT INTO books (author, title, year, isbn) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $bauthor, $btitle, $byear, $bisbn);
        ...
        // Close statement for Book(s)
        $stmt->close();
    }

    // Handle Book Chapter(s) data
    if(isset($_POST['bc_author']) && !empty($_POST['bc_author'])) {
        $bcauthors = $_POST['bc_author'];
        $bctitles = $_POST['bc_title'];
        $bcyears = $_POST['bc_year'];
        $bcisbns = $_POST['bc_isbn'];

        // Prepare and bind SQL statement for Book Chapter(s)
        $stmt = $conn->prepare("INSERT INTO book_chapters (author, title, year, isbn) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $bcauthor, $bctitle, $bcyear, $bcisbn);

        // Insert each book chapter into the database
        for ($i = 0; $i < count($bcauthors); $i++) {
            $bcauthor = $bcauthors[$i];
            $bctitle = $bctitles[$i];
            $bcyear = $bcyears[$i];
            $bcisbn = $bcisbns[$i];

            // Execute statement for Book Chapter(s)
            if (!$stmt->execute()) {
                echo "Error: " . $stmt->error;
                break; // Stop inserting if an error occurs
            }
        ...
        // Close statement for Book Chapter(s)
        $stmt->close();
    }

    // Retrieve form data for Google Scholar Link
    $google_link = $_POST["google_link"];

    // Prepare and bind SQL statement for Google Scholar Link
    $stmt = $conn->prepare("INSERT INTO google_scholar (google_link) VALUES (?)");
    $stmt->bind_param("s", $google_link);

    // Execute statement for Google Scholar Link
    if (!$stmt->execute()) {
        echo "Error: " . $stmt->error;
    }

    // Close statement for Google Scholar Link
    $stmt->close();

    // Redirect to the next page if all operations are successful
    header("Location: http://localhost/tutor/login5.php/");
...
<div class="col-md-1">
        <a href="http://localhost/tutor/login3.php/" class="btn btn-primary pull-left"><i class="fas fa-fast-backward"></i></a>
    </div>
